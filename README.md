...
还会显示任务累计 token。输入区只保留 `›` 或 `↳` 提示符，避免额外边框占用 transcript 空间。

模型正文默认都属于 Arc，不重复添加身份 gutter。完整行会原子提交到 transcript；尚未换行的 tail
以 50 ms 节流显示在 prompt_toolkit 管理的状态栏中，消息结束时再原子提交，因此不会和输入区重绘竞争。
任务正常完成后会留出一个空行，再显示下一次输入提示。非交互模式仍直接逐 fragment 输出。
工具原始输出不刷屏，完整结果可通过 `/last-tool` 查看。
